...
if not st.session_state["authenticated"]:
  ...
  with col1:
    ...
    st.write(
        "Manage inventory, sales, and digital billing seamlessly with 2+ years"
        " data storage."
    )
  with col2:
    with st.container():
  ...
  if menu == "Manage Inventory":
    ...
    st.write(
        "D.No 6/159/25, Pedda Harivanam Village, Adoni Mandal | Ph:"
        " 7995217343"
    )
    st.markdown("---")
    ...
    with col_c1:
      cust_name = st.text_input(
          "Customer / Farmer Name", value=default_cust_name
      )
    with col_c2:
    ...
    if inventory_df.empty:
      ...
      if len(st.session_state["cart"]) > 0:
        ...
        with col_act2:
          if st.button(
              "💾 Save & Generate Final Bill", use_container_width=True
          ):
    ...
    if not sales_df.empty:
      ...
      search_query = st.text_input(
          "Enter Bill No, Customer Name or Mobile No to Reprint:"
      )
      ...
      if search_query:
        ...
        if not filtered_bills.empty:
          ...
          selected_reprint_bill = st.selectbox(
              "Select Bill to Print", unique_matched_bills
          )
    ...
    with col_m1:
      st.metric(
          label="💵 Total Sales Collection", value=f"₹ {total_revenue:.2f}"
      )
    with col_m2:
